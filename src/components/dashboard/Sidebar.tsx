...
export default function Sidebar() {
  ...
  // 🔥 STATE UNTUK DATA PROFIL DINAMIS 🔥
  const [userData, setUserData] = useState({
    fullName: "Memuat...",
    role: "Memuat...",
    photoUrl: "/image/Logo AiMoo.png"
  });
      ...
      if (user) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Cek role dari farmRoles (Contoh: "Pemilik (Owner)" atau "Farm Manager")
            let roleName = "Pengguna";
            if (data.farmRoles) {
              const roles = Object.values(data.farmRoles);
              if (roles.length > 0) roleName = roles[0] as string;
            }

            setUserData({
              fullName: data.fullName || user.email || "Pengguna",
              role: roleName,
              photoUrl: data.photoUrl || "/image/Logo AiMoo.png"
            });
          }
        } catch (error) {
          console.error("Gagal memuat profil:", error);
          setUserData(prev => ({ ...prev, fullName: "Gagal memuat" }));
        }
      } else {
      ...
      <div className="p-4 border-t border-slate-100 space-y-4 bg-white shrink-0">
        ...
        <button className="w-full flex items-center gap-3 p-2 hover:bg-slate-50 rounded-xl transition-colors text-left border border-transparent hover:border-slate-100">
          <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
            {/* Pakai tag img biasa untuk menghindari error hostname next/image jika ambil URL dari Base64/Firebase */}
            <img src={userData.photoUrl} alt="Profile" className="w-full h-full object-cover" />
          </div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-bold text-slate-900 truncate">{userData.fullName}</p>
            <p className="text-xs font-medium text-slate-500 truncate capitalize">{userData.role}</p>
          </div>
